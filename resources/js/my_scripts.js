...
function loadStatsPage(){
	...
	var wins=0;
	var loses=0;
	var boulder="CU Boulder";
	for (rows=2; rows< table.rows.length; rows++)
	{
		if (parseInt(table.rows[rows].cells[2].innerHTML) < parseInt(table.rows[rows].cells[3].innerHTML)){
			loses++;
			table.rows[rows].cells[4].innerHTML = table.rows[rows].cells[1].innerHTML;
		}
		else{
			table.rows[rows].cells[4].innerHTML = boulder;
			wins++;
		}
...
function loadPlayersPage(){
	...
	for(i = 0; i < players.length; i++){
		names += '<a href="#" onclick = "switchPlayers('+i+')"> ' + players[i].name + '</a></br>' ;
	}
